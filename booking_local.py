"""
Booking.com – stahuje faktury a výkazy za předchozí měsíc, posílá emailem.
Před prvním spuštěním: spusť EXPORT_COOKIES.bat
"""
...
SESSION_FILE   = SCRIPT_DIR / "booking_session.json"
BOOKING_PROFILE = Path(r"C:\Users\jakes\booking_chrome_profile")
OUTPUT_DIR   = SCRIPT_DIR / "faktury"
...
PERIOD       = last_month.strftime("%Y-%m")    # "2026-03"
PERIOD_LABEL = last_month.strftime("%B %Y")    # "March 2026"
...
def load_cookies() -> list:
    if not SESSION_FILE.exists():
        print("CHYBA: booking_session.json nenalezen!")
        print("Spust EXPORT_COOKIES.bat")
        raise SystemExit(1)
...
async def get_invoice(page, context, hotel_id: int, ses: str) -> Path | None:
    ...
    sc = SCRIPT_DIR / "screenshots" / f"invoice_{hotel_id}.png"
    sc.parent.mkdir(exist_ok=True)
    await page.screenshot(path=str(sc), full_page=True)
...
async def get_statement(page, hotel_id: int, ses: str) -> Path | None:
    ...
    sc = SCRIPT_DIR / "screenshots" / f"statement_{hotel_id}.png"
    await page.screenshot(path=str(sc), full_page=True)
...
def _send_session_expired_email():
    msg = MIMEMultipart()
    msg["From"]    = SMTP_FROM
    ...
    msg.attach(MIMEText(
        ...
        "Prosím spusťte EXPORT_COOKIES.bat na svém počítači – přihlaste se jednou\n"
        "ručně a cookies se uloží pro příští měsíc.\n\n"
        "-- Automatická zpráva",
        "plain", "utf-8"
    ))
    with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
        server.login(SMTP_FROM, SMTP_PASSWORD)
        server.sendmail(SMTP_FROM, ["jakes.patrik@gmail.com"], msg.as_string())
    print("  Upozorneni o expiraci session odeslano.")

def send_email(hotel: dict, invoice: Path | None, statement: Path | None):
    name     = hotel["name"]
    hotel_id = hotel["id"]
    to_list  = hotel["to"] if isinstance(hotel["to"], list) else [hotel["to"]]
    cc       = hotel["cc"]
    ...
    if cc:
        msg["Cc"] = cc
    recipients = to_list + ([cc] if cc else [])

    has_docs = invoice or statement

    if has_docs:
        msg["Subject"] = f"Booking.com – {name} – {PERIOD_LABEL}"
        lines = [
            f"Dobrý den,\n",
            f"v příloze zasílám dokumenty z Booking.com za {PERIOD_LABEL}",
            f"pro ubytování: {name} (ID {hotel_id})\n",
        ]
        if invoice:
            lines.append(f"  • {invoice.name} – Commission invoice")
        else:
            lines.append(f"  • Faktura – nenalezena")
        if statement:
            lines.append(f"  • {statement.name} – Reservations statement")
        else:
            lines.append(f"  • Reservations statement – nenalezen")
        lines.append("\n-- Automatická zpráva")
    else:
        msg["Subject"] = f"Booking.com – {name} – {PERIOD_LABEL} – žádné rezervace"
        lines = [
            f"Dobrý den,\n",
...
async def main():
    ...
    if not BOOKING_PROFILE.exists():
        print("CHYBA: Booking Chrome profil nenalezen!")
        print(f"Spust EXPORT_COOKIES.bat a prihlaste se.")
        return
    ...
    async with async_playwright() as pw:
        ...
        if "sign-in" in page.url or "login" in page.url:
            print("Session expirovala – posilam upozorneni emailem...")
            _send_session_expired_email()
            await context.close()
            return
        ...
        if "ses=" in page.url:
            ...
            print(f"Prihlaseni OK (ses={ses[:8]}...)\n")
        else:
            print("Prihlaseni OK\n")
        ...
        for hotel in HOTELS:
            ...
            if invoice is None:
                print(f"  Zadna faktura -> posilam mail o zadnych rezervacich")
                statement = None
            else:
